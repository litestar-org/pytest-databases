...
def test_required_gate_accepts_intentional_skips(optional_status: str) -> None:
    failures = evaluate_required_jobs(
        ...
        expected={"quality": False, "compatibility": False, "docs": False, "provider": False},
    )
...
def test_required_gate_accepts_successful_expected_jobs() -> None:
    statuses = dict.fromkeys(("select", "quality", "compatibility", "docs", "provider"), "success")

    assert (
        evaluate_required_jobs(
            statuses,
            expected={"quality": True, "compatibility": True, "docs": True, "provider": True},
        )
        == []
    )
...
def test_required_gate_rejects_expected_provider_skip() -> None:
    ...
    assert evaluate_required_jobs(
        statuses,
        expected={"quality": True, "compatibility": True, "docs": False, "provider": True},
...
def test_required_gate_rejects_selector_failure_even_when_everything_else_skips() -> None:
    statuses = dict.fromkeys(("quality", "compatibility", "docs", "provider"), "skipped")
